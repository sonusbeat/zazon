<?php
$platillos = array(
	"chistorra" => array(
			"titulo" => "Entrada Chistorra",
			"img" => "entrada-chistorra.jpg",
			"des" => "",
			"back-link" => "/menu-entradas-ensaladas-carpacios-sopas.php"
	),
	"proboleta" => array(
		"titulo" => "Entrada Proboleta Asada",
		"img" => "entrada-proboleta-asada.jpg",
		"des" => "",
		"back-link" => "/menu-entradas-ensaladas-carpacios-sopas.php"
	),
	"tostadas-marlin" => array(
		"titulo" => "Entrada Tostadas de Marlin",
		"img" => "entrada-tostadas-marlin.jpg",
		"des" => "",
		"back-link" => "/menu-entradas-ensaladas-carpacios-sopas.php"
	),
	"tacos-pescado" => array(
		"titulo" => "Entrada Tacos de Pescado Ensenada",
		"img" => "entrada-tacos-pescado-ensenada.jpg",
		"des" => "",
		"back-link" => "/menu-entradas-ensaladas-carpacios-sopas.php"
	),
	"tacos-camaron" => array(
		"titulo" => "Entrada Tacos de Camarón al Pastor",
		"img" => "entrada-tacos-camaron-pastor.jpg",
		"des" => "",
		"back-link" => "/menu-entradas-ensaladas-carpacios-sopas.php"
	),
	"ensalada-mozzarella" => array(
		"titulo" => "Ensalada Mozzarella",
		"img" => "ensalada-motzarella.jpg",
		"des" => "",
		"back-link" => "/menu-entradas-ensaladas-carpacios-sopas.php"
	),
	"carpaccio-pulpo" => array(
		"titulo" => "Carpaccio de Pulpo",
		"img" => "carpaccio-pulpo.jpg",
		"des" => "",
		"back-link" => "/menu-entradas-ensaladas-carpacios-sopas.php"
	),
	"carpaccio-salmon" => array(
		"titulo" => "Carpaccio de Salmón",
		"img" => "carpaccio-salmon.jpg",
		"des" => "",
		"back-link" => "/menu-entradas-ensaladas-carpacios-sopas.php"
	),
	"carpaccio-pescado" => array(
		"titulo" => "Carpaccio de Pescado",
		"img" => "carpaccio-pescado.jpg",
		"des" => "",
		"back-link" => "/menu-entradas-ensaladas-carpacios-sopas.php"
	),
	"carpaccio-alcachofa" => array(
		"titulo" => "Carpaccio de Alcachofa",
		"img" => "carpaccio-alcachofa.jpg",
		"des" => "",
		"back-link" => "/menu-entradas-ensaladas-carpacios-sopas.php"
	),
	"crema-chile-poblano" => array(
		"titulo" => "Crema de Chile Poblano",
		"img" => "crema-chile-poblano.jpg",
		"des" => "",
		"back-link" => "/menu-entradas-ensaladas-carpacios-sopas.php"
	),
	"sopa-lima" => array(
		"titulo" => "Sopa de Lima",
		"img" => "sopa-lima.jpg",
		"des" => "",
		"back-link" => "/menu-entradas-ensaladas-carpacios-sopas.php"
	),
	"pasta-ignoquis-camarones" => array(
		"titulo" => "Iñoquis con Camarones a los 4 Quesos",
		"img" => "pasta-ignoquis-camarones.jpg",
		"des" => "Pasta a base de papa y harina, camarones sartenados, champiñones, espinacas, salsa cremosa, queso parmesano, manchego, queso azúl y mozzarella.",
		"back-link" => "/menu-pasta-carne-pollo-ceviche.php"
	),
	"pasta-lasagna-verde" => array(
		"titulo" => "Lasaña Verde",
		"img" => "pasta-lasagna-verde.jpg",
		"des" => "De carne bañada en salsa cremosa con espinacas.",
		"back-link" => "/menu-pasta-carne-pollo-ceviche.php"
	),
	"pasta-fetuccini-al-pesto-pollo" => array(
		"titulo" => "Fetuccini al Pesto con Pollo",
		"img" => "pasta-fetuccini-al-pesto-pollo.jpg",
		"des" => "Pasta fetuccini, salsa al pesto, tomate deshidratado, espinacas, pollo a la parrilla y queso parmesano.",
		"back-link" => "/menu-pasta-carne-pollo-ceviche.php"
	),
	"carne-filete-sammys" => array(
		"titulo" => "Filete Sammys",
		"img" => "carne-filete-sammys.jpg",
		"des" => "300 gr con cama de espinacas bañado con salsa Sammy's y crocante de camote.",
		"back-link" => "/menu-pasta-carne-pollo-ceviche.php"
	),
	"carne-filete-chemita" => array(
		"titulo" => "Filete Chemita",
		"img" => "carne-filete-chemita.jpg",
		"des" => "300gr con salsa de chile serrano, cebolla cambray, aceite de olica y salsa inglesa.",
		"back-link" => "/menu-pasta-carne-pollo-ceviche.php"
	),
	"carne-lomo-saltado" => array(
		"titulo" => "Lomo Saltado",
		"img" => "carne-lomo-saltado.jpg",
		"des" => "tiras de filete de res, julianas de cebolla morada, julianas de jitomate, chicaros, papas, cilantro, ají amarillo, acompañado acompañado de arroz al vapor.",
		"back-link" => "/menu-pasta-carne-pollo-ceviche.php"
	),
	"carne-steack-diana" => array(
		"titulo" => "Steack Diana",
		"img" => "carne-steack-diana.jpg",
		"des" => "450 gr con salsa de champiñones cremosa, con papa cambray y chicharos a la mantequilla.",
		"back-link" => "/menu-pasta-carne-pollo-ceviche.php"
	),
	"carne-ribeye-sammys" => array(
		"titulo" => "Rib Eye Sammy's",
		"img" => "carne-ribeye-sammys.jpg",
		"des" => "450 gr con salsa Sammy's, acompañado con papa cambray y esparragos.",
		"back-link" => "/menu-pasta-carne-pollo-ceviche.php"
	),
	"carne-cowboy-roberto" => array(
		"titulo" => "Cowboy Roberto",
		"img" => "carne-cowboy-roberto.jpg",
		"des" => "450 gr con cebollas, champiñones caramelizados, acompañados de papa cambray y ensalada.",
		"back-link" => "/menu-pasta-carne-pollo-ceviche.php"
	),
	"carne-arrachera" => array(
		"titulo" => "Arrachera Importada",
		"img" => "carne-arrachera.jpg",
		"des" => "350 gr acompañada de frijoles refritos, guacamole, salsa martajada, nopales, chiles, cebollas y tomates asados con torillas recién hechas.",
		"back-link" => "/menu-pasta-carne-pollo-ceviche.php"
	),
	"carne-vacio-tradicional" => array(
		"titulo" => "Vacio Tradicional",
		"img" => "carne-vacio-tradicional.jpg",
		"des" => "450 gr acompañado de papas cambray y ensalada.",
		"back-link" => "/menu-pasta-carne-pollo-ceviche.php"
	),
	"carne-vacio-florentino" => array(
		"titulo" => "Vacio Florentino",
		"img" => "carne-vacio-florentino.jpg",
		"des" => "450 gr con espinacas a la crema, migas de panco al romero acompañado de papas cambray.",
		"back-link" => "/menu-pasta-carne-pollo-ceviche.php"
	),
	"pollo-arroz-peruano" => array(
		"titulo" => "Arroz Peruano",
		"img" => "pollo-arroz-peruano.jpg",
		"des" => "Arroz frito con camarones y ají amarillo con pechuga de pollo.",
		"back-link" => "/menu-pasta-carne-pollo-ceviche.php"
	),
	"pollo-crispy-chicken" => array(
		"titulo" => "Crispy Chicken",
		"img" => "pollo-crispy-chicken.jpg",
		"des" => "Pollo sin hueso, con piel, puré de papa, espárragos y salsa obscura de pollo.",
		"back-link" => "/menu-pasta-carne-pollo-ceviche.php"
	),
	"ceviche-peruano" => array(
		"titulo" => "Ceviche Peruano",
		"img" => "ceviche-peruano.jpg",
		"des" => "Pescado blanco, ají amarillo, roccoto, elote, cebolla morada, cilantro, pimiento morrón rojo, acompañado con una media luna de camote.",
		"back-link" => "/menu-pasta-carne-pollo-ceviche.php"
	),
	"ceviche-chile-arbol" => array(
		"titulo" => "Ceviche Chile al de Arbol",
		"img" => "ceviche-chile-arbol.jpg",
		"des" => "Pescado blanco, cebolla morada, aguacate, cilantro, pepino y chile de árbol.",
		"back-link" => "/menu-pasta-carne-pollo-ceviche.php"
	),
	"ceviche-pulpo" => array(
		"titulo" => "Ceviche de Pulpo",
		"img" => "ceviche-pulpo.jpg",
		"des" => "Cebolla morada, aguacate, cilantro y crocantes de cebolla frita.",
		"back-link" => "/menu-pasta-carne-pollo-ceviche.php"
	),
	"ceviche-verde" => array(
		"titulo" => "Ceviche Verde",
		"img" => "ceviche-verde.jpg",
		"des" => "Pescado blanco, tomatillo verde, chile serrano, albahaca y cilantro.",
		"back-link" => "/menu-pasta-carne-pollo-ceviche.php"
	),
	"ceviche-acapulco" => array(
		"titulo" => "Ceviche Acapulco",
		"img" => "ceviche-acapulco.jpg",
		"des" => "Pescado blanco, salsa cátsup, cebolla, cilantro, jitomate, aguacate, aceitunas verdes y chile jalapeño.",
		"back-link" => "/menu-pasta-carne-pollo-ceviche.php"
	),
	"ceviche-tiritas-ixtapa" => array(
		"titulo" => "Ceviche Tiritas Ixtapa",
		"img" => "ceviche-tiritas-ixtapa.jpg",
		"des" => "Tiritas de pescado, cebolla, cebolla, chile serrano y aguacate.",
		"back-link" => "/menu-pasta-carne-pollo-ceviche.php"
	),
	"pescado-cajun" => array(
		"titulo" => "Pescado Cajún",
		"img" => "pescado-cajun.jpg",
		"des" => "Horneado, marinado con cajún a base de chiles secos, cama de esquites y guacamole acompañado con ensalada tropical.",
		"back-link" => "/menu-pescado-salmom-camarones-atun.php"
	),
	"pescado-zazon" => array(
		"titulo" => "Pescado Zazón",
		"img" => "pescado-zazon.jpg",
		"des" => "Horneado, bañado con mernier (tomate desidratado y albahaca) acompañado con puré de papa, y espárragos.",
		"back-link" => "/menu-pescado-salmom-camarones-atun.php"
	),
	"pescado-cucaracha" => array(
		"titulo" => "Pescado Cucuaracha",
		"img" => "pescado-cucaracha.jpg",
		"des" => "Sellado con salsa de chile de árbol tostado, acompáñado con puré de papa y espárragos.",
		"back-link" => "/menu-pescado-salmom-camarones-atun.php"
	),
	"pescado-zammys" => array(
		"titulo" => "Pescado Zammy's",
		"img" => "pescado-sammys.jpg",
		"des" => "Horneado, bañado con salsa Sammy's, acompañado de papa y espárragos.",
		"back-link" => "/menu-pescado-salmom-camarones-atun.php"
	),
	"salmon-piccata" => array(
		"titulo" => "Salmón Piccata",
		"img" => "salmon-piccata.jpg",
		"des" => "Salsa mernier con alcaparras, acompañado con puré de papa y espárragos.",
		"back-link" => "/menu-pescado-salmom-camarones-atun.php"
	),
	"salmon-sammys" => array(
		"titulo" => "Salmón Sammy's",
		"img" => "salmon-sammys.jpg",
		"des" => "Salsa Sammy's, fideos de sabanourus y vegetales.",
		"back-link" => "/menu-pescado-salmom-camarones-atun.php"
	),
	"salmon-zazon" => array(
		"titulo" => "Salmón Zazón",
		"img" => "salmon-zazon.jpg",
		"des" => "Constra de nuez sobre ragú de vegetales y salsa de mago.",
		"back-link" => "/menu-pescado-salmom-camarones-atun.php"
	),
	"camarones-momia" => array(
		"titulo" => "Camarones Momia",
		"img" => "camarones-momia.jpg",
		"des" => "Envueltos en tocino y queso manchego sobre rodaja de piña asada, con salsa de ciruela al chipotle, acompañados de ensalada arroz jazmín al vapor.",
		"back-link" => "/menu-pescado-salmom-camarones-atun.php"
	),
	"chiles-rellenos-camaron" => array(
		"titulo" => "Chile Relleno de Camarones",
		"img" => "chiles-rellenos-camaron.jpg",
		"des" => "Con salsa de chile guajillo, acompañado de arroz al vapor y ensalada.",
		"back-link" => "/menu-pescado-salmom-camarones-atun.php"
	),
	"camarones-zarandeados" => array(
		"titulo" => "Camarones Zarandeados",
		"img" => "camarones-zarandeados.jpg",
		"des" => "Adobados con adobo de la casa, rodajas de morrones y cebollas asadas, acompañados de arroz al vapor y ensalada.",
		"back-link" => "/menu-pescado-salmom-camarones-atun.php"
	),
	"atun-tropical" => array(
		"titulo" => "Atún Tropical",
		"img" => "atun-tropical.jpg",
		"des" => "Bañado con salsa de frutos rojos y un ralish de frutas de la estación y un toque de chile jalapeño.",
		"back-link" => "/menu-pescado-salmom-camarones-atun.php"
	),
	"atun-pastor" => array(
		"titulo" => "Atún Pastor",
		"img" => "atun-pastor.jpg",
		"des" => "Adobado con salsa al pastor montado sobre rodaja de piña asada, acompañado de salsa mexicana, papa cambray y ensalada.",
		"back-link" => "/menu-pescado-salmom-camarones-atun.php"
	),
	"atun-zazon" => array(
		"titulo" => "Atún Zazón",
		"img" => "atun-zazon.jpg",
		"des" => "Bañado en salsa mernier a las finas hierbas, acompañado de puré de papa y espárragos.",
		"back-link" => "/menu-pescado-salmom-camarones-atun.php"
	)
);
?>